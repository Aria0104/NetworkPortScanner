...
namespace NetworkPortScanner
{
    public partial class MainWindow : Window
    {
        ...
        private async void StartButton(object? sender, RoutedEventArgs e)
        {
            ...
            SemaphoreSlim semaphore = new SemaphoreSlim(Potoc); // ограничивает количество параллельных задач
            List<Task> tasks = new List<Task>();


            for (int port = Ot; port <= Do; port++)
            {
                await semaphore.WaitAsync(); // если достигнут лимит поток ждем
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    await Dispatcher.UIThread.InvokeAsync(() => //это объект, который управляет главным UI-потоком, отправляет задачу в UI поток
                    {
